...
<style>
    ...
    .portal-header {import streamlit as st
import pandas as pd
from datetime import datetime
from supabase import create_client
...
def get_sanitized_data():
    try:
        ...
        # 1. STANDARDIZE COLUMN NAMES (Fixes missing Email issue)
        # This converts everything like 'Email' or 'EMAIL' to lowercase 'email'
...
if not df.empty:
    ...
    # ALL FIELDS FROM YOUR TABS (Now as columns)
    all_requested_cols = [
    ...
    # KEYERROR FIX: Only select columns that actually exist in your Supabase
